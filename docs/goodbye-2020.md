...
* VSCode에서IntelliJ로 갈아탔다. 가장 큰 이유는 두 가지였다:
  * 브랜치를 변경 때마다 인덱싱을 새로하는데, 이때마다 프리징이 일어났다.
  * Vim 익스텐션이 오작동했다. `:w`를 입력하면 테마가 초기화되는게 제일 신박했다.
* 연초 지그재그UX그룹(구 커머스UX팀)으로 옮겼다:
  * 기능조직이 목적조직으로 바뀌고, 업무 방식이 충돌하며 처음엔 문제가 많았다. 
...
* **코로나 때문에 정말 사상 초유의 사태였다.**
  * [처음으로 재택근무를 해봤다.](https://news.joins.com/article/23715173)
  * 원래 밖에 잘 안 나가지만, 몇 개월 내내 전혀 나가지 않고 생활할 수 있었다. 배민 VIP가 됐다.
...
* 새 팀에서 코틀린 스프링 서버를 새롭게 구축했다:
  ...
  * 스프링과 코틀린을 써본적이 없어서 한달 동안 속성으로 공부하고 바로 프로덕션 개발을 했다.
* 사내 문서화TF 활동을 했다.
...
* **2종 보통 운전면허를 땄다:**
  ...
  * 코로나 때문에 딱히 갈 곳은 없고... 차에서 내리지 않고 그냥 주말마다 회사, 학교, 서울, 수원, 충북, 부산, 여기저기 막 돌아다녔다.
  * 정기적으로 서울에 있는 치과를 다니는데 굉장히 편해졌다.
* 업무에 매너리즘도 생기고, 여러모로 회의감이 들어서 의도적으로 주말에 개발을 하지 않았다.
* [내가 쓴 글이 판데믹 상황에서 도움이 됐다 :D](https://parksb.github.io/article/5.html)
  ![감사합니다. 코로나로 대학강의가 인강으로 대체되었는데, 자습에 한계를 느껴 참고하게 되었습니다. 굉장히 도움이 되었습니다.](https://pbs.twimg.com/media/EWIaUx7VcAkBXfY?format=jpg&name=medium)
* 새 언어 공부를 많이했다:
  * [TRPL](https://doc.rust-lang.org/book/) 완독하고, 토이 프로젝트도 몇개 했다.
  * 웹 프론트엔드도 러스트로 짜봤는데, 각종 UI 모듈을 쓰려니 결국 타입스크립트로 돌아왔다.
  * Elm, ReasonML도 써봤다. 같은 이유로 결국 타입스크립트로 돌아갔지만 나중엔 다시 바꾸고싶다.
* 코로나가 가라앉은 사이 친구들과 속초 여행을 갔는데...마법처럼 맹장염에 걸렸다.
  * 맹장염인지 모르고 밤새 뒹굴다가 아침에 응급실에 갔다. 너무 황당했다.
  * 속초에서 입원할 수 없어서 버스타고 서울로 왔다. 입원 가능한 병원을 찾아 떠돌다가 집 근처 병원에 갔다.
  * 태어나서 처음으로 수술받고 5일간 입원했다. 퇴원 다음날 업무 복귀하려니 엄청 우울했다.
* 바로 이어서 5개월간 매주 치과를 갔다. 크라운 처음해봤는데 너무 끔찍했다.
* 고통은 끝이 없다. 연말에는 갑자기 발톱이 살을 파고 들어서 치료를 시작했다.
* 추석 연휴동안 스파이더트론을 완성했다. 우주선보다 더 만족스럽다.
  ![Factorio spidertron](https://pbs.twimg.com/media/EjYCB8hUcAAhw-4?format=jpg&name=medium)
* 친구들이랑 어몽어스 정기 모임을 만들었다. 그냥 우기면 이기는 패턴이 반복돼서 결국 자연소멸.
* 팀 리더님이 생일선물로 용신목을 보내주셔서 키우기 시작했다. 
* 아이폰12를 샀다. 이렇게 좋은 제품을 애플같은 회사가 만들었다는게 슬프다.
* 작년부터 ETF 투자를 하다가 올해는 처음으로 직접 투자를 해봤다.
...
* [헌법재판소, "헌법재판소 결정 2017헌바127 형법 제269조 제1항 등 위헌소원", 2019](criminal-abortion.md)
...
블로그에는 하나밖에 못 썼다. 대신 사내 문서에 다 쏟았다. 

* [인터넷이 동작하는 아주 구체적인 원리](https://parksb.github.io/article/36.html)

## 본 영화

보건교사 안은영을 너무 보고 싶어서 넷플릭스를 잠시 구독했다. 추석 연휴에 하루 17시간씩 넷플릭스를 봤는데, 이러다간 인생 망할 것 같아서 끊어버렸다.
...
* Elephant Gym, "Finger", Balance, 2020
